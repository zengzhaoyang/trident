...
class resnet_v1_101_rcnn_trident_no(Symbol):
    def __init__(self):
        """
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        conv4 = self.get_resnet_v1_conv(conv3, 4, self.units[2], self.filter_list[2], conv4_var)
        
        conv5 = self.get_resnet_v1_conv(conv4, 5, self.units[3], self.filter_list[3], conv5_var, dilate=2, stride=1)

        rpn_cls_score, rpn_bbox_pred = self.get_rpn(conv4, num_anchors, 1, "")
        ...
        conv_new_1 = mx.sym.Convolution(data=conv5, kernel=(1, 1), num_filter=256, name="conv_new_1", weight=conv_new_1_weight, bias=conv_new_1_bias)
